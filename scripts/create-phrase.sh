...
for i in "${THRESHOLD[@]}"; do
    ...
    if [ STEP == 2 ]; then
      ...
      ./word2phrase -train "$DATASET" -output data_phrase_gram=${STEP}_th=${i}.txt -threshold "$i" -debug 2
    else
      echo "Creating phrases with threshold $i (step $STEP)"
      ./word2phrase -train data_phrase_gram=$((STEP - 1))_th=${THRESHOLD[STEP - 2]}.txt -output data_phrase_step=${STEP}_th=${i}.txt -threshold "$i" -debug 2
    fi
    STEP=$((STEP + 1))
done
